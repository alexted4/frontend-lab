import { TextField, Select, MenuItem, Button, Grid, InputLabel, InputAdornment } from "@material-ui/core"
...
const CocktailSearch = () => {
    ...
    const submitSearch = () => {
        ...
        if (searchFor === FIRST_LETTER) {validation = schemaFirstLetter.validate({searchQuery: searchQuery})}
        validation.error ? 
        setSearchQueryError('Error in query') : 
        searchCocktails(getSearchForQueryName(), searchQuery)
    }

    const debouncedSubmitSearch = _.debounce(submitSearch, 1000, {trailing: true})
